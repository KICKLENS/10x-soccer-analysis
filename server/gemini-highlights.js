...
가장 중요한 목표:
1. "선택된 선수"의 가치가 드러나는 장면만 우선 추출한다.
2. "우리 팀" 기준으로만 평가한다.
3. 상대팀 플레이를 따로 분석하지 않는다.
4. 선택 선수가 직접 관여했거나, 선택 선수의 판단/위치선정/반응/커뮤니케이션이 장면의 핵심일 때만 선택한다.
5. 단순 볼터치, 의미 없는 패스, 맥락 없는 장면, 비슷한 장면 반복은 제외한다.
6. 최종 결과는 "하이라이트 편집용 컷 리스트"여야 하며, 경기 전체 요약문이 아니라 실제 컷 편집에 쓸 수 있어야 한다.
...
- 수비수/골키퍼는 실점 방지, 커버, 차단, 라인 조율, 빌드업 판단이 드러나는 장면 우선
- 미드필더는 전개, 탈압박, 볼 회수, 방향 전환, 키패스 우선
- 공격수는 침투, 연계, 슈팅, 찬스 생성, 압박 유발 우선
- 상대팀 단독 장면은 제외
...
반드시 지켜야 할 분석 방향:
1. 우리 팀과 선택 선수 중심으로만 본다.
2. 상대팀 플레이를 따로 분석하지 않는다.
3. 선택 선수가 직접 관여했거나, 선택 선수의 판단/위치선정/반응/수비조율/빌드업 기여가 드러나는 장면만 뽑는다.
4. 단순 패스 교환, 의미 없는 터치, 비슷한 장면 반복, 애매한 장면은 제외한다.
5. 하이라이트는 편집용 컷 리스트이므로 "왜 넣어야 하는 장면인지"가 분명해야 한다.
6. 가능한 한 의미 있는 장면을 충분히 포착하되, 중복 없이 선별한다.
7. 장면 개수는 너무 적게 잡지 말고, 영상 길이에 맞게 ${limits.min}개 이상 ${limits.max}개 이하로 뽑아라.
8. 목표 개수는 약 ${limits.target}개다.
9. 각 장면은 4~18초 중심으로 제안한다.
10. summary와 playerFocus는 경기 전체 총평이 아니라 선택 선수의 하이라이트 편집 관점 요약이어야 한다.
