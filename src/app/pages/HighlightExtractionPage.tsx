...
export default function HighlightExtractionPage() {
  ...
  useEffect(() => {
    if (!payload?.highlightClips?.length) {
      setErrorMessage('먼저 영상 분석 페이지에서 「주요 장면 추출」을 완료해 주세요.');
    }
  ...
  const handleExtractHighlight = async () => {
    ...
    if (!clipList.length) {
      setErrorMessage('추출할 클립이 없습니다. 영상 분석 페이지에서 주요 장면을 먼저 추출해 주세요.');
      return;
    }
    ...
    try {
      ...
      setStatusMessage('하이라이트 영상이 준비됐습니다. 아래에서 재생·저장·공유할 수 있어요.');
    } catch (error) {
      ...
      <div className="mx-auto w-full max-w-[1200px] px-3 py-5 md:px-6 md:py-10 pb-[calc(28px+env(safe-area-inset-bottom))]">
        ...
        <div className="mt-4 mb-6">
          ...
          <p className="mt-3 max-w-[720px] text-sm leading-7" style={{ color: TEXT_SUB }}>
            영상 분석에서 뽑은 {clipCount}개 <strong className="text-white">주요 장면</strong>을 이어 붙이고,
            <strong className="text-white"> {playerLabel}</strong> 선수에게 코너 브라켓 추적 표시를 입힌
            <strong className="text-[#FFB648]"> SNS·공유용 하이라이트 영상</strong>을 만듭니다.
          </p>
        </div>
        ...
        <div className="rounded-3xl border p-5 md:p-6" style={{ borderColor: STROKE, background: CARD_BG }}>
          ...
          <div className="mt-5 grid gap-2 text-sm text-white/60">
            <div>주요 장면(클립): {clipCount}개</div>
            <div>하이라이트 상태: {spotlightUrl ? '추출 완료 ✓' : '「하이라이트 추출」 버튼을 눌러 주세요'}</div>
          </div>

          <div className="mt-6">
            {spotlightUrl ? (
              <div className="space-y-3">
                <h2 className="text-base font-bold md:text-lg">하이라이트 영상</h2>
                ...
                <HighlightSharePanel
                  ...
                  downloadName={payload?.jobId ? `highlight-${payload.jobId}.mp4` : '10x-highlight.mp4'}
              ...
              <div className="flex min-h-[280px] items-center justify-center rounded-2xl border border-white/10 bg-white/[0.02] px-6 text-center text-sm text-white/50">
                {clipCount
                  ? '「하이라이트 추출」 버튼을 누르면 선수 추적 표시가 입혀진 영상이 여기에 표시됩니다.'
                  : '영상 분석 페이지에서 먼저 「주요 장면 추출」을 진행해 주세요.'}
              </div>
            )}
          </div>
        </div>
